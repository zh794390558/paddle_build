...
struct DecoderOptions{
    ...
    // Please note the concept of ctc_scores in the following two search methods are different.
    // For CtcPrefixBeamSerch, it's a sum(prefix) score + context score
    // For CtcWfstBeamSerch, it's a max(viterbi) path score + context score
...
// DecodeResource is thread safe, which can be shared for multiple decoding threads
...
public:
    ...
    int frame_shift_in_ms() const{
        return model_->subsampling_rate() * 
            this->feature_frame_shift_in_ms();
    }
