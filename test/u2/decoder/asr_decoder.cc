...
DecodeState AsrDecoder::AdvanceDecoding(bool block){
    ...
    // compute frames need for chunk forward
    int num_requied_frames = model_->num_frames_for_chunk(start_);
    std::vector<std::vector<float>> chunk_feats;
    // Return immediately if we do not want to block
    if (!block && !feature_pipeline_->input_finished() && feature_pipeline_->NumQueuedFrames() < num_requied_frames){
        return DecodeState::kWaitFeats;
    }

    // if not okay, that means we reach the end of the input
    if (!feature_pipeline_->Read(num_requied_frames, &chunk_feats)){
            ...
            for (size_t j = 0; j < input.size(); j++){
                ...
                if (j > 0){
                    start = (time_stamp[j] - time_stamp[j-1]) * frame_shift_in_ms() < time_stamp_gap_ ?
                        (time_stamp[j-1] + time_stamp[j]) / 2 * frame_shift_in_ms() : start;
                }

                int end = time_stamp[j] * frame_shift_in_ms();
                if (j < input.size() -1){
                    end = (time_stamp[j+1] - time_stamp[j]) * frame_shift_in_ms() < time_stamp_gap_
                        ? (time_stamp[j+1] + time_stamp[j]) / 2 * frame_shift_in_ms() : end;
                }
...
void AsrDecoder::AttentionRescoring(){
    ...
    for (size_t i = 0; i < num_hyps; i++){
        result_[i].score = opts_.rescoring_weight * rescoring_score[i] + opts_.ctc_weight * result_[i].score;
    }
