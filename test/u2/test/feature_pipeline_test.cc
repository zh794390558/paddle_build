...
void pushQueue(const std::shared_ptr<ppspeech::BlockingQueue<int>>& que,
               std::vector<int> vec) {
  que->Push(std::move(vec));
}

void popQueue(const std::shared_ptr<ppspeech::BlockingQueue<int>>& que,
              int num,
              int back_data) {
...
TEST(FeaturePipelineTest, BlockingQueueTest) {
  auto capacity_queue = std::make_shared<ppspeech::BlockingQueue<int>>(2);
  std::vector<int> test_data{1, 2, 3, 4, 5};  // num > capacity
  std::thread push_thread(&pushQueue, capacity_queue, test_data);
  ASSERT_EQ(capacity_queue->Pop(), 1);
  ASSERT_LE(capacity_queue->Size(), 2);  // capacity_queue(capacity=2): 2 or 2,3
...
TEST(FeaturePipelineTest, PipelineTest) {
  ppspeech::FeaturePipelineConfig config(
      80, 8000, "cmvn");  // 80 fbank, 8k sample rate
